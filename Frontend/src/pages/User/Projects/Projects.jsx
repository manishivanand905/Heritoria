...
const ProjectsPage = () => {
  ...
  const filteredProjects = useMemo(() => {
    return projectsData.filter((project) => {
      ...
      const budgetFilter = budgetFilters.find((filter) => filter.id === selectedBudget);
      ...
      if (statusFilter && statusFilter.value && project.status !== statusFilter.value) {
        return false;
      }

      const areaFilter = areaFilters.find((filter) => filter.id === selectedArea);
      ...
      <PageWrapper>
        <PageContainer>
          <PageHeader>
            <h1>New Projects</h1>
            <ResultsCount>
              Explore {projectsData.length}+ curated residential projects in Hyderabad with
              exclusive benefits
            </ResultsCount>
          </PageHeader>
